...
def emotion_recognition(target_video_path, path_result, k_resolution, frame_use_rate, face_matching_th, target_image_path_list, model, model_name, emotions, split_video_index, split_video_num, known_faces):
    ...
    output_movie = cv2.VideoWriter('{}output.avi'.format(path_result), fourcc, video_frame_rate, (w, h))  # 出力する動画の詳細を設定する

    # CSVファイルの最初の列
    csv_saving_list = [[["frame_number", "time(ms)", "prediction", "x", "y", "absolute_x", "relative_x", "x_movement", "absolute_y", "relative_y", "y_movement", "mouse opening", "gesture", "top", "bottom", "left", "right"]] for _ in range(len(target_image_path_list))]
        ...
        if frame_number % frame_use_rate == 0 or first_flag:
            ...
            if frame_number % 30 == 0 or first_flag:  # 30フレームに一回のみ画像中の全領域から顔検出
                ...
                if face_location_list:
                    min_top, max_bottom = min([x[0] for x in face_location_list]), max([x[2] for x in face_location_list])
                ...
                face_location_list_trimmed = face_recognition.face_locations(rgb_frame_trimmed, model="hog")  # model="cnn"にすると検出率は上がるが10倍以上時間がかかる top,right,bottom,leftの順?
                face_location_list = []
            ...
            if face_location_list:
                for i in range(len(known_faces)):
                    if min(face_distance_list[i]) <= face_matching_th:
                        ...
                        elif face_distance_list[face_index_list[face_index]].index(min(face_distance_list[face_index_list[face_index]])) >= face_distance_list[i].index(min(face_distance_list[i])):  # クエリ画像が検出顔のいずれかとマッチング済みであるとき
                            face_index_list[face_index] = i
            ...
            for (top, right, bottom, left), face_index in zip(face_location_list, face_index_list):
                ...
                box_label(frame, left, top, right, bottom, "{}:{}".format(face_index, emotion[face_index]), color=(0, 255, 0))
                face_recog_flag[face_index] = True

                # 顔の部位の座標を検出する
                face_landmarks_list = face_recognition.face_landmarks(dst, face_locations=[(0, dst.shape[1], dst.shape[0], 0)])  # 顔の部位の座標が取れないときは return []
            ...
            for face_index in range(len(face_recog_flag)):
                if emotion[face_index] != "Unknown":
                    box_label(frame, int(rec_face[face_index][2]), int(rec_face[face_index][0]), int(rec_face[face_index][3]), int(rec_face[face_index][1]), "{}:{}".format(face_index, emotion[face_index]), color=(0, 255, 0))
        ...
        for face_index in range(len(known_faces)):
            if first_flag:  # 前フレームがない場合は終了
                ...
                if face_recog_flag[face_index]:
                    ...
                    # オプティカルフローのパラメータ設定
                    lk_params = dict(winSize=(int(rec_face[face_index][1] - rec_face[face_index][0]), int(rec_face[face_index][1] - rec_face[face_index][0])), maxLevel=2, criteria=(cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 10, 0.03))
                    p1[face_index], st, err = cv2.calcOpticalFlowPyrLK(frame_gray_old, frame_gray, p0[face_index], None, **lk_params)
                ...
                cv2.putText(frame, "{}".format(gesture_threshold), (face_index * 100, 30), cv2.FONT_HERSHEY_COMPLEX, 1.0, (0, 0, 0), 2)
                cv2.putText(frame, "{}".format(y_movement[face_index]), (face_index * 100, 60), cv2.FONT_HERSHEY_COMPLEX, 1.0, (0, 0, 0), 2)
    ...
    for i in range(len(target_image_path_list)):
        with open("{}result{}.csv".format(path_result, i), "w", encoding="Shift_jis") as f:
...
if __name__ == '__main__':
    ...
    emotions = ('Negative', 'Negative', 'Normal', 'Positive', 'Normal', 'Normal', 'Normal')  # emotions = ('Angry', 'Disgust', 'Fear', 'Happy', 'Neutral', 'Sad', 'Surprise')
    ...
    result_path = "result/{}_{}_{}_{}_{}_{}people/".format(datetime.date.today(), model_name, video_name, k_resolution, frame_use_rate, len(target_image_path_list))
    # 人の顔画像を読み込む
    known_faces = []
    ...
    for split_video_index in range(split_video_num):
        split_result_path = "result/{}_{}_{}_{}_{}_{}people/split_video_{}/".format(datetime.date.today(), model_name, video_name, k_resolution, frame_use_rate, len(target_image_path_list), split_video_index)
        cleanup_directory(split_result_path)
        process_list.append(multiprocessing.Process(target=emotion_recognition, args=(target_video_path, split_result_path, k_resolution, frame_use_rate, tolerance, target_image_path_list, model, model_name, emotions, split_video_index, split_video_num, known_faces, emotions_color_bgr_dict)))
        process_list[split_video_index].start()
    for process in process_list:
        process.join()
    # 分割して処理した動画を結合
    input_video_list = ["result/{}_{}_{}_{}_{}_{}people/split_video_{}/output.avi".format(datetime.date.today(), model_name, video_name, k_resolution, frame_use_rate, len(target_image_path_list), i) for i in range(split_video_num)]
    concat_video(input_video_list, result_path + "output.avi")
    for j in range(len(target_image_path_list)):
        input_csv_list = ["result/{}_{}_{}_{}_{}_{}people/split_video_{}/result{}.csv".format(datetime.date.today(), model_name, video_name, k_resolution, frame_use_rate, len(target_image_path_list), i, j) for i in range(split_video_num)]
