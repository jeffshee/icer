...
def match_speaker(path_diarization, path_emo_rec, th_matching=0.0):
    ...
    true_face_num = len([x for x in os.listdir(path_emo_rec) if ".csv" and "result" in x])

    # result0から順番にspeaker区切りごとの平均口開閉度を計算する
    for face_index in range(true_face_num):
        df_emotion = pd.read_csv("{}result{}.csv".format(path_emo_rec, face_index), encoding="shift_jis", header=0, usecols=["time(ms)", "mouse opening"])
        ...
        df_merged["mouse opening"] = (df_merged["mouse opening"] - df_merged["mouse opening"].mean(skipna=True)) / df_merged["mouse opening"].std(skipna=True)
        ...
        for speaker_class in range(len(df_diarization["speaker class"].unique())):
            ...
            mean_mouse_opening_in_each_speaker_class_list.append(df_merged_extracted1["mouse opening"].mean() if not pd.isnull(df_merged_extracted1["mouse opening"].mean()) else -100)
        ...
        for speaker_class in range(len(df_diarization["speaker class"].unique())):
            ...
            matching_similarity_list[speaker_class].append(df_merged_extracted1["mouse opening"].diff().abs().mean() * max_rate)

    for speaker_index, similarity in enumerate(matching_similarity_list):
        print("{}th speaker".format(speaker_index), similarity)

    matching_index_list = [matching_similarity.index(max(matching_similarity)) if max(matching_similarity) >= th_matching else -1 for matching_similarity in matching_similarity_list]
