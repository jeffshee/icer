...
def cluster_face(target_image_path, output_path, cluster_num):
    for cluster_index in range(cluster_num):
        cleanup_directory(output_path + "{}/".format(cluster_index))

    face_image_name_list = os.listdir(target_image_path)
    face_image_name_list = [f for f in face_image_name_list if os.path.isfile(os.path.join(target_image_path, f))]  # ファイル名のみの一覧を取得

    # 画像を読み込みエンコード
    face_image_encoded_list = []
    for face_image_name in face_image_name_list:
        face_image = face_recognition.load_image_file(target_image_path + face_image_name)
    ...
    closest, _ = pairwise_distances_argmin_min(model.cluster_centers_, face_image_encoded_np)  # Get nearest point to centroid

    # クラスタリング結果ごとにインデックスを付け保存
    for i, (label, face_image_name) in enumerate(zip(pred_labels, face_image_name_list)):
        if i == closest[label]:
            shutil.copyfile(target_image_path + face_image_name,
                            output_path + "{}/{}.PNG".format(label, "closest"))
        else:
            shutil.copyfile(target_image_path + face_image_name,
                            output_path + "{}/{}.PNG".format(label, i))
