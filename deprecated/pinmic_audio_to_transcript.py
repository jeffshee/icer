...
def pinmic_audio_to_transcript(pinmic_audio_name_list, path_diarization, path_audio, path_result, matching_index_list):
    ...
    for (pinmic_audio_name, matching_index) in zip(pinmic_audio_name_list, matching_index_list):
        ...
        split_wave_per_sentence(df_diarization_sorted, df_split_ms["time(ms)"].values.tolist(),
                                path_audio + pinmic_audio_name + ".wav",
                                path_result + "{}/split_wave/".format(pinmic_audio_name))
    ...
    wav_to_transcript(path_result + "selected_wav/",
                      split_ms_list=df_split_ms["time(ms)"].values.tolist() + [audio_duration_ms],
                      output_path=path_result + "transcript.csv")


if __name__ == '__main__':
    path_result = "pinmic_to_transcript/"
    path_audio = "audio/"
    pinmic_audio_name_list = ["200225_Haga_22_voice1", "200225_Haga_22_voice2", "200225_Haga_22_voice3",
                              "200225_Haga_22_voice4", "200225_Haga_22_voice5"]

    # pinmic_mix_audioの作成
    mixed_audio_name = pinmic_audio_name_list[0].split("pinmic_")[0] + "pinmic_mix"
    mix_audio([path_audio + pinmic_audio_name + ".wav" for pinmic_audio_name in pinmic_audio_name_list],
              path_audio + mixed_audio_name + ".wav")
    ...
    pinmic_audio_to_transcript(pinmic_audio_name_list, path_diarization_result, path_audio, path_result,
                               matching_index_list)
