...
def csv_to_text(csv_path, output_path, specific_column, specific_index=None):
    ...
    if specific_index:
        for i in df_csv[specific_index].unique():
            ...
            df_csv_extracted.to_csv(output_path.replace(".txt", "_{}.txt".format(i)), sep=' ', index=False, header=False, columns=specific_column, encoding="utf_8_sig")
...
if __name__ == "__main__":
    pass
    # csv_to_text("tmp/transcript.csv", "tmp.txt", ["Text"])
